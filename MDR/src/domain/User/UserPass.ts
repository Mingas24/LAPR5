...
export class UserPass extends ValueObject<UserPassProps> {
  get value(): string {
  ...
  public static create(props: UserPassProps): Result<UserPass> {
    const propsResult = Guard.againstNullOrUndefined(props.value, "password");

    if (!propsResult.succeeded) {
      return Result.fail<UserPass>(propsResult.message);
    } else {
      if (!props.hashed) {
        if (!this.isAppropriateLength(props.value)) {
          return Result.fail<UserPass>(
            "Password doesnt meet criteria [1 uppercase, 1 lowercase, one digit or symbol and 8 chars min]."
          );
        }
      }

      return Result.ok<UserPass>(
        new UserPass({
          value: props.value,
