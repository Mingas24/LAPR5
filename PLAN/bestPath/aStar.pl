...
aStar(Orig,Dest,TimeToCatch,Cam,Cost):-aStar2(Dest,[(_,TimeToCatch,[Orig])],Cam,Cost).

aStar(Orig,Dest,TimeToCatch,Cam,Cost):-get_time(Ti),
                                      aStar2(Dest,[(_,TimeToCatch,[Orig])],Cam,Cost),
                                      get_time(Tf),
                                      TSol is Tf-Ti.
                                    %   write('Solution Time:'),write(TSol),nl.
    
aStar2(Dest,[(_,EndTime,[Dest|T])|_],Cam,EndTime):- reverse([Dest|T],Cam).
aStar2(Dest,[(_,TimeToCatch,LA)|Other],Cam,Cost):-
            LA=[Act|_],
